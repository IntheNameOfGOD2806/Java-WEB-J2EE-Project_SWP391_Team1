...
public class BlogListController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (service.equals("changeStatus")) {
            ...
            int blogIDi = Integer.parseInt(blogID);
            Boolean statusBoo = Boolean.parseBoolean(status);
            if (statusBoo == true) {
                blogDAO.changeStatusById(blogIDi, 0);
            } else {
                blogDAO.changeStatusById(blogIDi, 1);
            }
            BL = blogDAO.getAllBlog();
        }
        if (service.equals("sort")) {
            String sortType = (String) request.getParameter("type");
            BL = blogDAO.getSortedBlog(sortType);
            numPs = BL.size();
            numperPage = 3;
            ...
            request.setAttribute("dataBL", BL);
        }
